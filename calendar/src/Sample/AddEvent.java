...
/**
 * Graphics for pop up menu that allows user to add a new event event.
 * Contains options to create event title, description, date, start/end time,
 * and color code event. References css file event_stylesheet.css
 *
 * Written by Toni Eidmann, Claudia Naughton, and Kyra Wilson
 * 11/20/17
 */
public class AddEvent{
    ...
    public AddEvent(EventControllerInterface controller, EventModelInterface model, DailyObserver observer){
        this.controller = controller;
        this.model = model;
        this.observer = observer;
    }

    /** Creates scene, adds elements to window. Additionally, runs through the process of submitting new event
     * information to controller. */
    public void start(Stage primaryStage) {
        VBox outline = new VBox();
        //VBox that contains all addEvent menu items
        Scene scene = new Scene(outline, 550, 325);
        scene.getStylesheets().add("src/event_stylesheet.css");
        primaryStage.setScene(scene);
        primaryStage.show();
        primaryStage.setTitle("Add Event Menu");

        Text title = new Text("Add Event:");
        title.getStyleClass().add("textHeader");
        HBox addEventHeader = new HBox(title);
        addEventHeader.setAlignment(Pos.TOP_CENTER);
        ...
        titleBox.getChildren().addAll(eTitle, eventInput);
        ...
        HBox startTimeBox = startTime("Start:");
        startTimeBox.setPadding(new Insets(10));

        HBox endTimeBox1 = endTime("End:");
                            ...
                            if (amButtonEnd.isSelected()) {
                                ...
                                break CHECK;
                                        ...
                                        try {
                                            ...
                                            Date startTime = new SimpleDateFormat("HH:mm a",
                                                    Locale.ENGLISH).parse(start);
                                            Date endTime = new SimpleDateFormat("HH:mm a",
                                                    Locale.ENGLISH).parse(end);
        ...
        saveCancelBox.getChildren().addAll(cancel, save);
        saveCancelBox.setPadding(new Insets(10));
        saveCancelBox.setSpacing(5);
        saveCancelBox.setAlignment(Pos.BOTTOM_RIGHT);
        outline.getChildren().addAll(addEventHeader, titleBox, descrBox, startTimeBox, endTimeBox1,
    ...
    private HBox setCategory() {
        ...
        //simulates toggle functionality for checkboxes
    ...
    private HBox startTime(String timeLabel) {
        ...
        LocalDate currentNonRealTimeDate = model.getEventDate();
        DatePicker datePicker = new DatePicker(currentNonRealTimeDate);
...
//    
    private HBox endTime(String timeLabel) {
